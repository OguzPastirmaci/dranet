...
func init() {
	...
	flag.StringVar(&cloudProviderOptions, "cloud-provider-options", "", "Comma-separated <provider>.<option>=<value> pairs for the active cloud provider. Requires an explicit --cloud-provider-hint. Values must not contain secrets; flags are logged.")
...
func setupProviders(ctx context.Context, opts providerOptions) (cloudprovider.CloudInstance, cloudprovider.ProfileProvider, error) {
	...
	if cloudOptions := opts.dependencies.ProviderOptions; len(cloudOptions) > 0 {
		...
		switch discovery.CloudProviderHint(opts.cloudProviderHint) {
		case discovery.CloudProviderHintGCE, discovery.CloudProviderHintAWS, discovery.CloudProviderHintAzure,
			discovery.CloudProviderHintOKE, discovery.CloudProviderHintAlibaba, discovery.CloudProviderHintCKS,
			discovery.CloudProviderHintWebhook:
		default:
			return nil, nil, fmt.Errorf("cloud provider options require a canonical --cloud-provider-hint (GCE, AWS, AZURE, OKE, ALIBABA, CKS, webhook), got %q", opts.cloudProviderHint)
		}
		for key := range cloudOptions {
			namespace, _, _ := strings.Cut(key, ".")
			if !strings.EqualFold(namespace, opts.cloudProviderHint) {
				return nil, nil, fmt.Errorf("cloud provider option %q does not match --cloud-provider-hint=%s", key, opts.cloudProviderHint)
			}
		}
	}
