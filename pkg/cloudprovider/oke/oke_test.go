...
// fakeRail describes one more interface on the node under test.
...
// The profile resolution of rail 0 on the test node with the OCA ARP policy.
var testRail0Config = &apis.NetworkConfig{
...
func TestGetDeviceConfig(t *testing.T) {
	...
	rail := cloudprovider.DeviceIdentifiers{Name: "rdma0", PCIAddress: "0000:0c:00.0"}
	tests := []struct {
		name         string
		ifName       string
		pciAddress   string
		...
		{
			name:         "Ethernet rail gets the profile",
			ifName:       "rdma0",
			pciAddress:   "0000:0c:00.0",
		...
		{
			name:         "renamed rail with an OCA address gets the profile",
			ifName:       "ens3f0",
			pciAddress:   "0000:0c:00.0",
			hardwareType: unix.ARPHRD_ETHER,
		...
		{
			name:         "renamed rail is ignored on a node with rdmaN names",
			ifName:       "ens3f0",
			pciAddress:   "0000:0c:00.0",
			hardwareType: unix.ARPHRD_ETHER,
			addresses:    []string{"10.224.13.19/12"},
		...
		{
			name:         "InfiniBand rail gets no profile",
			ifName:       "rdma0",
			pciAddress:   "0000:0c:00.0",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			instance.addressFallback = !tt.namedRails
...
func TestGetProfileConfig(t *testing.T) {
	...
	rail0 := testRail0Config
	rail0AddressOnly := &apis.NetworkConfig{Interface: rail0.Interface}
	...
	tests := []struct {
		...
		},
		{
			...
			metadata: ipv4Fabric(),
			ifName:   "rdma0",
			wantErr:  "does not move an RDMA rail into a pod",
		},
		{
		...
		{
			name:      "other rail without an address is skipped",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "rdma0",
			addresses: []string{"10.224.13.19/12"},
			extra:     []fakeRail{{ifName: "rdma1", hardwareType: unix.ARPHRD_ETHER, addresses: []string{}}},
			want:      rail0,
		},
		{
		...
		},
		{
			...
			config:    profileConfig(apis.InterfaceConfig{VRF: &apis.VRFConfig{Name: "rail"}}),
			metadata:  ipv4Fabric(),
			ifName:    "rdma0",
			addresses: []string{"10.224.13.19/12"},
			want:      rail0AddressOnly,
		...
		{
			name:      "rail name and address disagree",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "rdma1",
			addresses: []string{"10.224.13.19/12"},
		...
		{
			name:      "rail outside the child range",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "rdma16",
		...
		{
			name:      "renamed rail resolved by address inversion",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "ens3f0",
			addresses: []string{"10.224.13.19/12"},
		...
		{
			name:      "renamed last rail resolved by address inversion",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "ens3f1",
			addresses: []string{"10.225.237.19/12"},
		...
		{
			name:      "renamed rail without an OCA address yet",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "ens3f0",
			addresses: []string{"10.140.77.19/19"},
		...
		{
			name:      "renamed rail with an address of another host",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "ens3f0",
		...
		{
			name:      "unknown address mode rejects another rail with a wrong address",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "rdma0",
			addresses: []string{"10.224.13.19/12"},
			extra:     []fakeRail{{ifName: "rdma1", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.224.13.20/12"}}},
		...
		{
			name:      "unknown address mode rejects a renamed rail with a wrong remainder",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
			ifName:    "rdma0",
			addresses: []string{"10.224.13.19/12"},
			extra:     []fakeRail{{ifName: "ens3f1", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.224.13.20/12"}}},
			wantErr:   "could not validate the OCA classic IPv4 layout: interface ens3f1: address 10.224.13.20 does not match",
		},
		{
			name:      "known classic mode checks the claimed rail only",
			config:    profileConfig(apis.InterfaceConfig{}),
			metadata:  ipv4Fabric(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, rail := range tt.extra {
				fakeInterface(t, rail.ifName, "", rail.hardwareType)
				if rail.addresses != nil {
					fakeInterfaceAddresses(t, rail.ifName, rail.addresses...)
				}
			}
...
func TestGetProfileConfigValidatesLayoutOnce(t *testing.T) {
	...
	// A consistent host passes once, and later drift on another rail is not rechecked.
...
func TestHasFabricParent(t *testing.T) {
	t.Run("named rail", func(t *testing.T) {
		fakeSysfs(t)
		instance := newOKEInstance(nil, nil)
		if hasNamedRail() || instance.hasFabricParent() {
			t.Fatal("a rail was found without a sysfs directory")
		}
		fakeInterface(t, "eth0", "", unix.ARPHRD_ETHER)
		fakeInterfaceAddresses(t, "eth0", "10.140.77.19/19")
		if hasNamedRail() || instance.hasFabricParent() {
			t.Fatal("a rail was found with only the VCN interface")
		}
		fakeInterface(t, "rdma3", "", unix.ARPHRD_INFINIBAND)
		if hasNamedRail() || instance.hasFabricParent() {
			t.Fatal("a rail was found with only native InfiniBand")
		}
		fakeInterface(t, "rdma4", "", unix.ARPHRD_ETHER)
		if !hasNamedRail() || !instance.hasFabricParent() {
			t.Fatal("no rail was found with an Ethernet rdmaN interface")
		}
	})

	t.Run("renamed rail with an OCA address", func(t *testing.T) {
		fakeSysfs(t)
		instance := newOKEInstance(nil, nil)
		fakeInterface(t, "ens3f0", "", unix.ARPHRD_ETHER)
		fakeInterfaceAddresses(t, "ens3f0")
		if hasNamedRail() || instance.hasFabricParent() {
			t.Fatal("a rail was found for an Ethernet interface without addresses")
		}
		fakeInterfaceAddresses(t, "ens3f0", "10.224.13.19/12")
		if hasNamedRail() {
			t.Fatal("hasNamedRail() = true for a renamed rail")
		}
		if !instance.hasFabricParent() {
			t.Fatal("hasFabricParent() = false for an Ethernet interface in the OCA network")
		}
		// A node with rdmaN names does not probe addresses.
		instance.addressFallback = false
		if instance.hasFabricParent() {
			t.Fatal("hasFabricParent() = true for a renamed rail without the address fallback")
		}
	})
}

func TestFabricInterfaceIndex(t *testing.T) {
	tests := []struct {
		ifName    string
		wantIndex int
		wantOK    bool
	...
	for _, tt := range tests {
		t.Run(tt.ifName, func(t *testing.T) {
			gotIndex, gotOK := fabricInterfaceIndex(tt.ifName)
			if gotIndex != tt.wantIndex || gotOK != tt.wantOK {
				t.Errorf("fabricInterfaceIndex(%q) = %d, %v, want %d, %v", tt.ifName, gotIndex, gotOK, tt.wantIndex, tt.wantOK)
			}
		})
	}
}
...
func TestDeriveRailIPv4(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "rail 0 on a /19 subnet", vnic: testVNIC(), railIndex: 0, parentRange: defaultRange, wantParent: "10.224.13.19", wantChild: "10.222.13.19"},
		{name: "rail 1 on a /19 subnet", vnic: testVNIC(), railIndex: 1, parentRange: defaultRange, wantParent: "10.224.45.19", wantChild: "10.222.45.19"},
		{name: "rail 8 on a /19 subnet", vnic: testVNIC(), railIndex: 8, parentRange: defaultRange, wantParent: "10.225.13.19", wantChild: "10.223.13.19"},
		{name: "rail 15 on a /19 subnet", vnic: testVNIC(), railIndex: 15, parentRange: defaultRange, wantParent: "10.225.237.19", wantChild: "10.223.237.19"},
		{name: "rail 16 on a /19 subnet", vnic: testVNIC(), railIndex: 16, parentRange: defaultRange, wantErr: "rail 16 with a /19 node subnet is outside the child range 10.222.0.0/15"},
		{name: "rail 1 on a /20 subnet", vnic: vnicIn("10.140.64.0/20"), railIndex: 1, parentRange: defaultRange, wantParent: "10.224.29.19", wantChild: "10.222.29.19"},
		{name: "rail 7 on a /18 subnet", vnic: vnicIn("10.140.64.0/18"), railIndex: 7, parentRange: defaultRange, wantParent: "10.225.205.19", wantChild: "10.223.205.19"},
		{name: "rail 8 on a /18 subnet", vnic: vnicIn("10.140.64.0/18"), railIndex: 8, parentRange: defaultRange, wantErr: "rail 8 with a /18 node subnet is outside the child range 10.222.0.0/15"},
		{name: "rail 0 in a /16 OCA network", vnic: testVNIC(), railIndex: 0, parentRange: netip.MustParsePrefix("192.168.0.0/16"), wantParent: "192.168.13.19", wantChild: "10.222.13.19"},
		{name: "rail 8 in a /16 OCA network", vnic: testVNIC(), railIndex: 8, parentRange: netip.MustParsePrefix("192.168.0.0/16"), wantErr: "rail 8 with a /19 node subnet is outside the OCA RDMA network 192.168.0.0/16"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			parent, child, err := deriveRailIPv4(tt.vnic, tt.railIndex, tt.parentRange)
			if tt.wantErr != "" {
				if err == nil || !strings.Contains(err.Error(), tt.wantErr) {
					t.Fatalf("deriveRailIPv4() error = %v, want %q", err, tt.wantErr)
				}
				return
			}
			if err != nil {
				t.Fatalf("deriveRailIPv4() returned error: %v", err)
			}
			if parent.String() != tt.wantParent || child.String() != tt.wantChild {
				t.Errorf("deriveRailIPv4() = %s, %s, want %s, %s", parent, child, tt.wantParent, tt.wantChild)
			}
		})
	}
}

func TestRailIndexFromAddress(t *testing.T) {
	defaultRange := netip.MustParsePrefix(okeRDMAParentIPv4CIDR)
	tests := []struct {
		name    string
		vnic    *primaryVNIC
		...
		{name: "rail 0", vnic: testVNIC(), address: "10.224.13.19", want: 0},
		{name: "rail 1", vnic: testVNIC(), address: "10.224.45.19", want: 1},
		{name: "rail 15", vnic: testVNIC(), address: "10.225.237.19", want: 15},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := railIndexFromAddress(tt.vnic, netip.MustParseAddr(tt.address), defaultRange)
			if tt.wantErr != "" {
				if err == nil || !strings.Contains(err.Error(), tt.wantErr) {
					t.Fatalf("railIndexFromAddress() error = %v, want %q", err, tt.wantErr)
				}
				return
			}
			if err != nil {
				t.Fatalf("railIndexFromAddress() returned error: %v", err)
			}
			if got != tt.want {
				t.Errorf("railIndexFromAddress() = %d, want %d", got, tt.want)
			}
		})
	}
}
...
func TestValidateClassicHostLayout(t *testing.T) {
	tests := []struct {
		...
		{name: "no rails", wantErr: "no Ethernet RDMA rail holds an OCA address yet"},
		{
			name: "named rail without an address is skipped",
			rails: []fakeRail{
		...
		{
			name:       "renamed rails are ignored on a node with rdmaN names",
			rails:      []fakeRail{{ifName: "ens3f0", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.224.13.19/12"}}},
			namedRails: true,
			wantErr:    "no Ethernet RDMA rail holds an OCA address yet",
		},
		{
			name: "consistent named rails",
			rails: []fakeRail{
				{ifName: "rdma0", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.224.13.19/12"}},
		...
		{
			name:    "named rail with a wrong address",
			rails:   []fakeRail{{ifName: "rdma1", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.224.13.19/12"}}},
			wantErr: "interface rdma1 has address 10.224.13.19, but the derived OCA address is 10.224.45.19",
		},
		{
			name:    "only rail without an address",
			rails:   []fakeRail{{ifName: "rdma0", hardwareType: unix.ARPHRD_ETHER, addresses: []string{}}},
			wantErr: "no Ethernet RDMA rail holds an OCA address yet",
		},
		{
			name:    "address read error",
			rails:   []fakeRail{{ifName: "rdma0", hardwareType: unix.ARPHRD_ETHER}},
			wantErr: "could not read addresses for rdma0",
		},
		{
			name:    "renamed rail with a wrong remainder",
			rails:   []fakeRail{{ifName: "ens3f0", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.224.13.20/12"}}},
			wantErr: "interface ens3f0: address 10.224.13.20 does not match",
		},
		{
			name:    "named rail outside the child range",
			rails:   []fakeRail{{ifName: "rdma16", hardwareType: unix.ARPHRD_ETHER, addresses: []string{"10.226.13.19/12"}}},
			wantErr: "could not derive the OCA address for rdma16",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
// railNodeServer serves the IMDS responses of the B4.8 test node and counts
// the requests per endpoint. vnics answers the vnics requests in order; the
// last entry repeats.
func railNodeServer(t *testing.T, requests map[string]*atomic.Int64, vnics ...func(w http.ResponseWriter)) *httptest.Server {
	t.Helper()
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func newRailNodeRequests() map[string]*atomic.Int64 {
...
func TestStartPollsUntilPrimaryVNICIsKnown(t *testing.T) {
	fakeSysfs(t)
	fakeInterface(t, "rdma0", "0000:0c:00.0", unix.ARPHRD_ETHER)
	requests := newRailNodeRequests()
	server := railNodeServer(t, requests,
...
func TestRefreshReadsVNICOnlyUntilKnown(t *testing.T) {
	...
	fakeInterface(t, "rdma0", "0000:0c:00.0", unix.ARPHRD_ETHER)
	requests := newRailNodeRequests()
	server := railNodeServer(t, requests, serveVNIC)
...
func TestRefreshPicksUpLateRail(t *testing.T) {
	fakeSysfs(t)
	requests := newRailNodeRequests()
	server := railNodeServer(t, requests, serveVNIC)
	...
	if got := requests["/vnics/"].Load(); got != 0 {
		t.Fatalf("start() made %d vnics requests on a node without rails", got)
	}
	id := cloudprovider.DeviceIdentifiers{Name: "rdma0", PCIAddress: "0000:0c:00.0"}
	config := &apis.NetworkConfig{Profile: okeRDMAProfile}
	...
	// The rail appears after startup with its OCA address and ARP policy.
	fakeInterface(t, "rdma0", "0000:0c:00.0", unix.ARPHRD_ETHER)
	fakeInterfaceAddresses(t, "rdma0", "10.224.13.19/12")
	fakeARP(t, "rdma0", "1", "2")
	...
	if diff := cmp.Diff(testRail0Config, got); diff != "" {
		t.Errorf("GetProfileConfig() mismatch (-want +got):\n%s", diff)
	}
}
...
func TestStartWarnsInExclusiveNetnsMode(t *testing.T) {
	...
	fakeNetnsMode(t, "N")
	logs := captureLogs(t)
	server := railNodeServer(t, newRailNodeRequests(), serveVNIC)
