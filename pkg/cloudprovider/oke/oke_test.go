...
func TestGetProfileConfig(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var options []Option
			if tt.childRange != "" {
				options = append(options, WithChildIPv4Range(netip.MustParsePrefix(tt.childRange)))
			}
...
func TestParseChildIPv4Range(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "shortest prefix length", value: "10.0.0.0/8", want: prefix(10, 0, 0, 0, 8)},
		{name: "range for a /18 subnet", value: "10.192.0.0/14", want: prefix(10, 192, 0, 0, 14)},
		{name: "range for a /24 subnet", value: "10.192.0.0/20", want: prefix(10, 192, 0, 0, 20)},
		{name: "longest prefix length", value: "10.192.0.0/30", want: prefix(10, 192, 0, 0, 30)},
		{name: "range outside 10.0.0.0/8", value: "172.16.0.0/12", want: prefix(172, 16, 0, 0, 12)},
		{name: "prefix length below the limit", value: "8.0.0.0/7", wantErr: "must have a prefix length from 8 to 30"},
		{name: "prefix length above the limit", value: "10.192.0.0/31", wantErr: "must have a prefix length from 8 to 30"},
		{name: "prefix length above 32", value: "10.192.0.0/33", wantErr: "prefix length out of range"},
		{name: "not in masked form", value: "10.193.0.0/14", wantErr: "is not in masked form, use 10.192.0.0/14"},
		{name: "IPv6 range", value: "fd00::/16", wantErr: "is not an IPv4 prefix"},
		{name: "address without a prefix length", value: "10.192.0.0", wantErr: "no '/'"},
		{name: "empty value", value: "", wantErr: "no '/'"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := ParseChildIPv4Range(tt.value)
			if tt.wantErr != "" {
				if err == nil || !strings.Contains(err.Error(), tt.wantErr) {
					t.Fatalf("ParseChildIPv4Range(%q) error = %v, want %q", tt.value, err, tt.wantErr)
				}
				if got.IsValid() {
					t.Errorf("ParseChildIPv4Range(%q) returned %s with an error", tt.value, got)
				}
				return
			}
			if err != nil {
				t.Fatalf("ParseChildIPv4Range(%q) returned error: %v", tt.value, err)
			}
			if got != tt.want {
				t.Errorf("ParseChildIPv4Range(%q) = %s, want %s", tt.value, got, tt.want)
			}
		})
	}
}
...
func TestGetInstanceAppliesOptions(t *testing.T) {
	tests := []struct {
		...
		{name: "child range option", childRange: "10.192.0.0/14", want: netip.PrefixFrom(netip.AddrFrom4([4]byte{10, 192, 0, 0}), 14)},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fakeSysfs(t)
