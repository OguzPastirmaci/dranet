...
const (
	...
	// okeRDMAProfile is advertised for every Ethernet RDMA rail and resolved
	// by this provider into an IPvlan child with a deterministic address.
	okeRDMAProfile = "oke-rdma"
	// okeRDMASBRTableBase plus the rail index is the routing table of a child.
	// TableIDForName hashes into 1000 to 1999 and can collide between two
	// rails of one pod; the rail index cannot.
...
// the RDMA rail addresses from them.
...
type okeMetadata struct {
	...
	// PrimaryVNIC is read once, on a node with Ethernet RDMA rails. It is nil
...
type OKEInstance struct {
	...
	// addressFallback identifies rails by their OCA address on a node whose
	// rails do not carry the rdmaN name.
...
// Ethernet RDMA rail, so the rail never moves into a pod. A rail on an IPv6
...
func (o *OKEInstance) GetDeviceConfig(id cloudprovider.DeviceIdentifiers) *apis.NetworkConfig {
	...
	if err != nil || !o.isFabricParent(ifName) {
		return nil
	}
...
func (o *OKEInstance) GetProfileConfig(id cloudprovider.DeviceIdentifiers, _ *resourceapi.ResourceClaim, config *apis.NetworkConfig) (*apis.NetworkConfig, error) {
	...
	index, err := railIndex(ifName, addressIn(addresses, parentRange), vnic, parentRange)
	if err != nil {
		return nil, err
	}
	parentAddr, childAddr, err := deriveRailIPv4(vnic, index, parentRange)
	if err != nil {
...
// not support. A rail stays on the host, so a passthrough request is an
...
func validateOKEProfileRequest(iface *apis.InterfaceConfig) error {
	if iface.Type == apis.InterfaceTypePassthrough {
		return errors.New("the OKE profile does not move an RDMA rail into a pod; remove interface.type: Passthrough")
	}
...
// railIndex returns the rail index from the rdmaN name, or inverts it from
// the live OCA address of a rail without that name.
func railIndex(ifName string, address netip.Addr, vnic *primaryVNIC, parentRange netip.Prefix) (int, error) {
	if index, ok := fabricInterfaceIndex(ifName); ok {
		return index, nil
	}
	if !address.IsValid() {
		return 0, fmt.Errorf("interface %s has no address in the OCA RDMA network %s yet", ifName, parentRange)
	}
	index, err := railIndexFromAddress(vnic, address, parentRange)
	if err != nil {
		return 0, fmt.Errorf("interface %s: %w", ifName, err)
	}
	klog.V(2).Infof("OKE rail index %d of %s comes from its address %s", index, ifName, address)
	return index, nil
}

// parentARPPolicy fills the ARP settings the claim leaves unset from the
// parent, so the child answers ARP the way OCA configured the rail.
...
// only while the primary VNIC is unknown on a node with rails. A host or VNIC
...
func (o *OKEInstance) start(ctx context.Context, client *http.Client, endpoint string) (*OKEInstance, error) {
	o.fetchMetadata = func(ctx context.Context) (*okeMetadata, error) {
		...
		// rails. The rail check repeats, because a rail can appear or get its
		// OCA address after startup.
		current := o.metadata.Load()
		needVNIC := (current == nil || current.PrimaryVNIC == nil) && o.hasFabricParent()
		return fetchOKEMetadata(ctx, client, endpoint, needVNIC)
	}
	// The OCA files are static per boot. Read them once, before the rail
	// check, because the parent range identifies unnamed rails.
	o.ocaConfig = loadOCARDMAConfig(ocaConfigDir)
	o.addressFallback = !hasNamedRail()
	// The driver rejects IPvlan children in exclusive mode. Say so once here
	// instead of on the first failed claim.
	if o.hasFabricParent() && rdmaExclusiveNetnsMode() {
		klog.Warningf("The RDMA subsystem runs in exclusive network namespace mode; the OKE profile creates IPvlan children, so rail claims fail until the node uses shared mode (ib_core netns_mode=1)")
	}
...
// hasNamedRail reports whether the node has an Ethernet rdmaN interface.
func hasNamedRail() bool {
	entries, err := os.ReadDir(sysClassNet)
	if err != nil {
		return false
	}
	for _, entry := range entries {
		if _, ok := fabricInterfaceIndex(entry.Name()); !ok {
			continue
		}
...
// hasFabricParent reports whether the node has an Ethernet RDMA rail.
func (o *OKEInstance) hasFabricParent() bool {
	entries, err := os.ReadDir(sysClassNet)
	if err != nil {
		return false
	}
	for _, entry := range entries {
		if o.isFabricParent(entry.Name()) {
			return true
		}
	}
	return false
...
// isFabricParent reports whether ifName is an Ethernet RDMA rail: it has the
// rdmaN name or, on a node without such names, an address in the OCA RDMA
// network. The VCN NICs are Mellanox devices too, so hardware is not enough.
func (o *OKEInstance) isFabricParent(ifName string) bool {
	hardwareType, err := interfaceHardwareType(ifName)
	if err != nil || hardwareType != unix.ARPHRD_ETHER {
		return false
	}
	if _, ok := fabricInterfaceIndex(ifName); ok {
		return true
	}
...
// fabricInterfaceIndex returns the rail index encoded in an rdmaN name. OCA
// assigns the name and the rail address from the same table entry.
func fabricInterfaceIndex(ifName string) (int, bool) {
	digits, ok := strings.CutPrefix(ifName, fabricInterfacePrefix)
	if !ok {
		return 0, false
	}
...
// validateClassicHostLayout checks every rail that holds an OCA address
// against the classic formula. One rail can match by coincidence under
...
func (o *OKEInstance) validateClassicHostLayout(vnic *primaryVNIC) error {
	...
	for _, entry := range entries {
		...
		if !address.IsValid() {
			// OCA has not configured this rail yet.
			continue
		}
		index, err := railIndex(ifName, address, vnic, parentRange)
		if err != nil {
			return err
		}
		expected, _, err := deriveRailIPv4(vnic, index, parentRange)
		if err != nil {
	...
	if checked == 0 {
		return errors.New("no Ethernet RDMA rail holds an OCA address yet")
	}
	return nil
}
...
// deriveRailIPv4 computes the OCA parent and the Dranet child address of one
// rail. OCA classic mode assigns
// parent = base + railIndex*subnetSize + (vnicIP - vnicSubnetBase),
// and the child keeps the parent offset inside the child range.
func deriveRailIPv4(vnic *primaryVNIC, railIndex int, parentRange netip.Prefix) (netip.Addr, netip.Addr, error) {
	if err := validateVNIC(vnic); err != nil {
		return netip.Addr{}, netip.Addr{}, err
	}
	if !parentRange.IsValid() || !parentRange.Addr().Is4() || parentRange != parentRange.Masked() {
		return netip.Addr{}, netip.Addr{}, fmt.Errorf("OCA RDMA network %s is not a masked IPv4 prefix", parentRange)
	}
	...
	if subnet.Overlaps(parentRange) {
		return netip.Addr{}, netip.Addr{}, fmt.Errorf("primary VNIC subnet %s overlaps the OCA RDMA network %s", subnet, parentRange)
	}
	// A VNIC subnet inside the child range would collide with child addresses.
	if subnet.Overlaps(childRange) {
		return netip.Addr{}, netip.Addr{}, fmt.Errorf("primary VNIC subnet %s overlaps the Dranet child range %s", subnet, childRange)
	}

	blockSize := uint64(1) << (32 - subnet.Bits())
	hostPosition := uint64(ipv4ToUint32(vnic.IPv4) - ipv4ToUint32(subnet.Addr()))
	offset := uint64(railIndex)*blockSize + hostPosition
	if offset >= uint64(1)<<(32-parentRange.Bits()) {
		return netip.Addr{}, netip.Addr{}, fmt.Errorf("rail %d with a /%d node subnet is outside the OCA RDMA network %s", railIndex, subnet.Bits(), parentRange)
	}
	if offset >= uint64(1)<<(32-childRange.Bits()) {
		return netip.Addr{}, netip.Addr{}, fmt.Errorf("rail %d with a /%d node subnet is outside the child range %s", railIndex, subnet.Bits(), childRange)
	}
	parent := uint32ToIPv4(ipv4ToUint32(parentRange.Addr()) + uint32(offset))
	child := uint32ToIPv4(ipv4ToUint32(childRange.Addr()) + uint32(offset))
	return parent, child, nil
}

// railIndexFromAddress inverts the classic formula for a parent without an
...
func railIndexFromAddress(vnic *primaryVNIC, address netip.Addr, parentRange netip.Prefix) (int, error) {
	if err := validateVNIC(vnic); err != nil {
		return 0, err
	}
