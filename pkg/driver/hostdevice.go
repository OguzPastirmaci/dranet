...
func nsAttachNetdev(hostIfName string, containerNsPAth string, interfaceConfig apis.InterfaceConfig, hostNetworkConfig apis.NetworkConfig, hostIPv4Sysctls *InterfaceIPv4Sysctls) (*resourceapi.NetworkDeviceData, error) {
	...
	if err := applyInterfaceIPv4SysctlsInNamespace(containerNsPAth, ifName, hostIPv4Sysctls); err != nil {
		_, rollbackErr := nsDetachNetdev(containerNsPAth, ifName, hostNetworkConfig, hostIPv4Sysctls)
...
func nsDetachNetdev(containerNsPAth string, devName string, hostNetworkConfig apis.NetworkConfig, hostIPv4Sysctls *InterfaceIPv4Sysctls) (bool, error) {
	containerNs, err := netns.GetFromPath(containerNsPAth)
	if err != nil {
	...
	ifName := attrs.Name
	if hostNetworkConfig.Interface.Name != "" {
		ifName = hostNetworkConfig.Interface.Name
	}
	nameData := nl.NewRtAttr(unix.IFLA_IFNAME, nl.ZeroTerminated(ifName))
	req.AddData(nameData)
	...
	return true, restoreHostInterfaceConfig(ifName, hostNetworkConfig, hostIPv4Sysctls)
}

func restoreHostInterfaceConfig(ifName string, hostNetworkConfig apis.NetworkConfig, hostIPv4Sysctls *InterfaceIPv4Sysctls) error {
	...
	if err = netlink.LinkSetUp(hostDev); err != nil {
		...
		hostDev, err = nlwrap.LinkByName(ifName)
		if err != nil {
			errorList = append(errorList, fmt.Errorf("failed to refresh link for interface %s: %w", ifName, err))
		} else if hostDev.Attrs().RawFlags&unix.IFF_UP == 0 {
			errorList = append(errorList, fmt.Errorf("interface %s is still down after restoration", ifName))
		}
	}

	if err := restoreHostRoutingConfig(ifName, hostNetworkConfig); err != nil {
		errorList = append(errorList, fmt.Errorf("failed to restore routing for interface %s: %w", ifName, err))
	}
